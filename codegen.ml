...
let translate (globals, functions) =
  ...
  let ltype_of_typ = function
    ...
    | A.Double -> float_t
    | A.Void   -> void_t
    | A.Char   -> i8_t  (* Char uses 1 byte *)
    | _        -> i8_t (* Other types are all now mapped to i8_t  TODO: map other types to LLVM types. *)
    (* P.S. For now, it seems to me that, implementing matrix/vector will be quite difficult *)
  in
  ...
  let build_function_body fdecl =
	...
	SLiteral i  -> L.const_int i32_t i
      ...
      | SDoubleliteral l -> L.const_float_of_string float_t l
    ...
    | A.Cr      -> L.build_fadd (* TODO: Write a part for Cross Product. Seems difficult *)
    | A.Dot     -> L.build_fadd (* TODO: Write a part for Dot Product. Seems difficult *)
	  ) e1' e2' "tmp" builder
      | SBinop (e1, op, e2) ->
	  let e1' = expr builder e1
	  and e2' = expr builder e2 in
    ...
    | A.Dot     -> L.build_fadd (* TODO: Write a part for Dot Product. Seems difficult *)
	  ...
	  | A.Neg                  -> L.build_neg
    | A.Inverse      -> L.build_neg (* TODO: Write a part for Inverse. Seems difficult *)
    | A.Transpose    -> L.build_neg (* TODO: Write a part for Transpose. Seems difficult *)
